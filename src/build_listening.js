...
const R = (text,opts={}) => new TextRun({text:String(text||''),font:TNR,size:24,...opts});
const expandBlanks = t => String(t||'').replace(/_{2,}/g, m => '_'.repeat(Math.max(20, m.length)));
const P = (children,opts={}) => new Paragraph({spacing:{before:0,after:80,line:276},children,...opts});
...
function buildListening(data) {
  ...
  (data.part1||[]).forEach(q => {
    ...
    (q.options||[]).forEach(opt => ch.push(P([R('     '+opt)],{spacing:{before:0,after:20,line:276}})));
  });
